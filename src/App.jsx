...
// Configurações do JSON Bin - VERSÃO ANTIGA ($2a$)
const JSON_BIN_CONFIG = {
  BIN_ID: '68fcf497ae596e708f2be3c4', // SEU BIN ID
  API_KEY: '$2a$10$5/zTLS/5BfaK5.j9M0FJkex/9BHF.QNttD2rmg146l00j6MAuPWr6'
};
...
// Funções para JSON Bin (versão antiga)
const saveDataToJSONBin = async (data) => {
  try {
    // Usando proxy CORS
    const proxyUrl = 'https://cors-anywhere.herokuapp.com/';
    const targetUrl = `https://api.jsonbin.io/b/${JSON_BIN_CONFIG.BIN_ID}`;
    
    const response = await fetch(proxyUrl + targetUrl, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
        'secret-key': JSON_BIN_CONFIG.API_KEY,
    ...
    if (!response.ok) {
      throw new Error('Erro ao salvar no JSON Bin');
    }
    
    console.log('Dados salvos no JSON Bin via proxy!');
    return await response.json();
  } catch (error) {
    console.error('Erro ao salvar no JSON Bin:', error);
...
const loadDataFromJSONBin = async () => {
  try {
    // Usando proxy CORS
    const proxyUrl = 'https://cors-anywhere.herokuapp.com/';
    const targetUrl = `https://api.jsonbin.io/b/${JSON_BIN_CONFIG.BIN_ID}/latest`;
    ...
    if (!response.ok) {
      throw new Error('Erro ao carregar do JSON Bin');
    }
    
    const data = await response.json();
    console.log('Dados carregados do JSON Bin via proxy!');
    return data;
  } catch (error) {
    console.error('Erro ao carregar do JSON Bin, usando localStorage:', error);
    ...
    return {
      ...
      socialMedia: JSON.parse(localStorage.getItem('socialMedia') || 'null')
    };
  }
};
...
export default function App() {
  ...
  // Carregar dados do JSON Bin ou localStorage
  useEffect(() => {
    const loadAllData = async () => {
      setIsLoading(true);
      try {
        const remoteData = await loadDataFromJSONBin();
        ...
        if (remoteData.companyInfo) {
          setCompanyInfo(remoteData.companyInfo);
        } else {
          const saved = localStorage.getItem('companyInfo');
          setCompanyInfo(saved ? JSON.parse(saved) : initialCompanyInfo);
        }
        
        if (remoteData.socialMedia) {
          setSocialMedia(remoteData.socialMedia);
        } else {
          const saved = localStorage.getItem('socialMedia');
          setSocialMedia(saved ? JSON.parse(saved) : initialSocialMedia);
        }
      } catch (error) {
        ...
        setCompanyInfo(savedCompanyInfo ? JSON.parse(savedCompanyInfo) : initialCompanyInfo);
        setSocialMedia(savedSocialMedia ? JSON.parse(savedSocialMedia) : initialSocialMedia);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const saveAllData = async () => {
    ...
    try {
      await saveDataToJSONBin(dataToSave);
    } catch (error) {
  ...
  const validateProduct = (product) => {
    ...
    if (!product.price || parseFloat(product.price.replace(',', '.')) <= 0) errors.price = 'Preço deve ser maior que zero';
    ...
    if (editingProduct) {
      ...
      const updatedProducts = products.map(p => 
        p.id === editingProduct.id ? { 
          ...editingProduct, 
          price: parseFloat(editingProduct.price.replace(',', '.')) 
        } : p
      );
  ...
  const handleUpdateSocialMedia = () => {
    ...
    showAlert('Redes sociais atualizadas com sucesso!');
  };
  ...
  // ... (mantenha todas as outras funções render* existentes)
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center mb-16">
            <div>
              <h1 className="text-5xl lg:text-6xl font-bold mb-8">
                Nossa <span className="bg-gradient-to-r from-[#6b5a12] via-[#8e7a18] to-[#a8922c] bg-clip-text text-transparent block">História</span>
              </h1>
  ...
  // Botão Admin VISÍVEL em todos os dispositivos
  const renderHeader = () => (
        ...
        <div className="flex justify-between items-center py-4">
          ...
          <div className="flex items-center space-x-4">
            <button
              ...
              className="bg-gold-gradient text-black px-6 py-2 rounded-xl font-semibold hover:shadow-lg transform hover:scale-105 transition-all duration-300"
            >
              Área Admin
            </button>
  ...
  // ... (mantenha TODAS as outras funções render* que você já tem)
    ...
    return (
      <div className="min-h-screen bg-dark-gradient flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gold-light">Carregando dados...</p>
        </div>
      </div>
    );
  }
